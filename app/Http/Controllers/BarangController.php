...
class BarangController extends Controller
{
    ...
    {
        // Query barang dengan relasi kategori dan produk
        $query = Barang::with(['kategori', 'produk']);

        // Cek apakah ada filter tanggal pembelian
        if ($request->filled('tanggal_awal') && $request->filled('tanggal_akhir')) {
            $query->whereBetween('tanggal_pembelian', [$request->tanggal_awal, $request->tanggal_akhir]);
        }

        // Ambil data barang setelah filter diterapkan
        $barang = $query->get();

        // Ambil data kategori dan produk untuk dropdown
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'tanggal_pembelian' => 'required|date', // Validasi tanggal pembelian
        ]);
        ...
        $barang->tanggal_pembelian = $request->tanggal_pembelian; // Simpan tanggal pembelian
        $barang->save();

        return redirect()->back()->with('success', 'Barang berhasil ditambahkan!');
    }

    // Fungsi untuk membuat kode barang unik
